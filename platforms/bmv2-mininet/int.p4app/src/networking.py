...
def start_int_collector(influxdb):
    ...
    collector_cmd = 'ip netns exec ns_int python3 /tmp/utils/int_collector_influx.py -i 6000 -H 192.168.0.1:8086 -d 1 > /tmp/p4app_logs/int_collector_stdout.log 2>&1'
    print(collector_cmd)
    # FIX: antes se usaba os.system(collector_cmd), que ESPERA a que el
    # comando termine antes de continuar. Con el colector roto (InfluxDB
    # inalcanzable) esto no se notaba porque el proceso fallaba y moria
    # enseguida. Ahora que el colector conecta de verdad y se queda
    # escuchando para siempre, os.system() se quedaba bloqueado ahi y el
    # arranque nunca llegaba a "Ready!". Con subprocess.Popen se lanza en
    # segundo plano, igual que ya se hace un poco mas arriba con socat.
    subprocess.Popen(collector_cmd, shell=True)
