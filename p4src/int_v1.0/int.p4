...
control ingress(inout headers hdr, inout metadata meta, inout standard_metadata_t ig_intr_md) {
    apply {
        // Descartar paquetes que no sean TCP, UDP ni ICMP.
        // FIX: la condicion original no incluia ICMP, por eso el ping
        //      era descartado antes de llegar a Int_source.
        if (!hdr.udp.isValid() && !hdr.tcp.isValid() && !hdr.icmp.isValid())
            exit;

        // Si el puerto de ingreso tiene INT source activo, anadir cabeceras INT
        Int_source.apply(hdr, meta, ig_intr_md);

        // Reenvio L2: seleccionar puerto de salida segun MAC destino
        Forward.apply(hdr, meta, ig_intr_md);
        PortForward.apply(hdr, meta, ig_intr_md);

        // En el nodo sink: clonar el paquete I2E para generar reporte INT
...
control egress(inout headers hdr, inout metadata meta, inout standard_metadata_t eg_intr_md) {
    apply {
        // Nodo transit: anadir metadata INT del nodo actual
        Int_transit.apply(hdr, meta, eg_intr_md);
        // Nodo sink: eliminar cabeceras INT del paquete original
        //           o reformatear el clon como reporte INT para el colector
