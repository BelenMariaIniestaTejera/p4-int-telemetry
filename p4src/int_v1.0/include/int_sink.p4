...
control Int_sink(inout headers hdr, inout metadata meta, in egress_intrinsic_metadata_t standard_metadata, in egress_intrinsic_metadata_from_parser_t imp) {
    ...
    action remove_sink_header() {
        ...
        hdr.int_pkt_len.setInvalid();
        
        // remove int data
        // FIX ICMP: antes aqui se ponia hdr.icmp.checksum = 16w0 a mano,
        // dejando un checksum invalido tras quitar las cabeceras INT.
        // Se recalcula correctamente en computeChecksum (parser.p4),
        // ya sin cabeceras INT (porque hdr.int_header ya estara invalido
        // en ese punto), asi que no hace falta tocar el checksum aqui.
        if (hdr.icmp.isValid()) {
            meta.int_metadata.remove_int = 1w1;
        }
        hdr.int_shim.setInvalid();
        hdr.int_header.setInvalid();
        // ARREGLO metadata acumulada: ahora hdr.int_data SI se ha rellenado
        // en el parser (ver parser.p4), asi que invalidarlo aqui tiene
        // efecto real: el deparser dejara de emitirlo, y con el desaparecen
        // de una vez todos los hops anteriores que antes se quedaban sin
        // borrar (el bug que detectamos con las capturas).
