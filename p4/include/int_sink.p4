...
const bit<32> INT_REPORT_MIRROR_SESSION_ID = 1;   // Mirror session used for cloned INT report packets
#ifdef BMV2
...
control Int_sink_config(inout headers hdr, inout metadata meta, inout ingress_intrinsic_metadata_for_tm_t standard_metadata) {
    ...
    action configure_sink(bit<16> sink_reporting_port) {
        meta.int_metadata.remove_int = 1;    // Mark packet for INT header removal at egress
    ...
    // Table used to enable INT sink functionality on a specific egress port
    table tb_int_sink {
        actions = {
            configure_sink;
        }
...
control Int_sink(inout headers hdr, inout metadata meta, in egress_intrinsic_metadata_t standard_metadata, in egress_intrinsic_metadata_from_parser_t imp) {
    ...
    action remove_sink_header() {
        ...
        // remove int data
        // ICMP checksum is recalculated later in ComputeChecksum after INT removal.
        if (hdr.icmp.isValid()) {
            meta.int_metadata.remove_int = 1w1;
        }
        hdr.int_shim.setInvalid();
        hdr.int_header.setInvalid();
        // Remove accumulated INT metadata before forwarding the original packet.
