...
// Register used to store the report sequence number
#ifdef BMV2
...
// [ETH][IP][UDP][INT REPORT HDR][ETH][IP][UDP/TCP/ICMP][INT SHIM][INT DATA]
...
// Report length: outer and original IPv4 headers, outer UDP header,
// original Ethernet header, INT report header and INT data
#ifdef BMV2
            hdr.report_ipv4.totalLen = (bit<16>)(20 + 20 + 8 + 14)
                + ((bit<16>)(INT_REPORT_HEADER_LEN_WORDS)<<2)
                + (((bit<16>)hdr.int_shim.len) << 2);
                
			// add size of original transport header
			if (hdr.tcp.isValid()) {
            ...
            // Remove data beyond the generated INT report
#ifdef BMV2
