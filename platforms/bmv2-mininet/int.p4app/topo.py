...
def main():
    ...
    # Desactivar verificacion checksum ICMP en todos los hosts
    for host in net.hosts:
        host.cmd('ip link set {}-eth0 promisc on'.format(host.name))
        host.cmd('ethtool -K {}-eth0 rx off tx off gso off gro off lro off'.format(host.name))
        host.cmd('sysctl -w net.ipv4.conf.all.rp_filter=0'.format(host.name))
