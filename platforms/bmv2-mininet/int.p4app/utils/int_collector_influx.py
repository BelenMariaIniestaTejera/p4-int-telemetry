...
class HopMetadata:
    def __init__(self, data, ins_map, int_version=1):
        ...
        self.__parse_pkt_len()  # ANADIDO: siempre presente, no usa ningun bit de ins_map
    ...
    def __parse_pkt_len(self):
        # campo propio (no estandar), anadido siempre, sin bit de mascara
        self.pkt_len = int.from_bytes(self.data.read(4), byteorder='big')
        logger.debug('parse pkt_len: %d' % self.pkt_len)
...
class IntReport():
    def __init__(self, data):
        ...
        if protocol == 17:  # UDP
            l4_hdr = data[50:58]
            scrp = struct.unpack('!H', l4_hdr[:2])[0]
            dstp = struct.unpack('!H', l4_hdr[2:4])[0]
            ...
            scrp = 0
            dstp = 0
            self.icmp_type = l4_hdr[0]
            self.icmp_code = l4_hdr[1]
        ...
        if protocol == 1:
            self.flow_id['icmp_type'] = self.icmp_type
            self.flow_id['icmp_code'] = self.icmp_code
        ...
        if protocol == 17:
            ...
            offset = offset + ICMP_OFFSET
        else:
            raise Exception("Unsupported original protocol %s in embedded packet" % protocol)    
...
class IntCollector():
    ...
    def __prepare_hop_report(self, report, index, hop, flow_key):
        ...
        if "pkt_len" in vars(hop):
            json_report["fields"]["pkt_len"] = hop.pkt_len
