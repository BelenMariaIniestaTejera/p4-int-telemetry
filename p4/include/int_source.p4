...
// Register used to store the INT header sequence number
#ifdef BMV2
...
control Int_source(inout headers hdr, inout metadata meta, in ingress_intrinsic_metadata_t standard_metadata, in ingress_intrinsic_metadata_from_parser_t imp) {
    ...
    // Configure the INT source parameters.
    // max_hop: maximum number of INT nodes that may add metadata
    // hop_metadata_len: metadata length added by each INT node, in 4-byte words
    // ins_mask: specifies which INT metadata fields are collected
    action configure_source(bit<8> max_hop, bit<5> hop_metadata_len, bit<5> ins_cnt, bit<16> ins_mask) {
       ...
       // Preserve the original DSCP value and mark the packet as INT
        hdr.int_shim.dscp = hdr.ipv4.dscp;
        hdr.ipv4.dscp = IPv4_DSCP_INT;

        // Update the IPv4 total length after INT header insertion
        hdr.ipv4.totalLen = hdr.ipv4.totalLen + INT_ALL_HEADER_LEN_BYTES;
        
         // Update UDP length when applicable
        if (hdr.udp.isValid()) {
            hdr.udp.len = hdr.udp.len + INT_ALL_HEADER_LEN_BYTES;
        }
        
        // ICMP checksum is recalculated later in ComputeChecksum.
    }

    // Select flows to be monitored with INT.
    // ICMP flows use l4_proto = 0x01 and zero-valued L4 ports.
    table tb_int_source {
        actions = {
            configure_source;
        }
    ...
    // Enable INT source functionality on a specific ingress port
    ...
    apply {
        ...
        // Check whether INT source functionality is enabled on the ingress port
